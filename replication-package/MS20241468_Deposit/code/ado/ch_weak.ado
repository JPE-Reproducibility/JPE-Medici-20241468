* ---------------------------------------------------------------------------
* Modified from Goldsmith-Pinkham, Sorkin & Swift (bartik-weight) ch_weak.ado:
* the absorb() branch below uses `areg` rather than the upstream `reg ...,
* absorb()`, which is not a valid regress option and cannot run. `areg` absorbs
...
program define ch_weak, rclass
	...
	foreach beta in `beta_range' {
		...
		if "`absorb'" != "" {
			qui areg `y_tilde' `z' `controls' `weight', absorb(`absorb') `se'
			}
		else {
